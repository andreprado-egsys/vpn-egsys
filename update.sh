...
# >>> vpn-egsys >>>
vpnro() { snxctl disconnect 2>/dev/null; sleep 1; cp ~/.config/snx-rs/vpnro.conf ~/.config/snx-rs/snx-rs.conf; snxctl connect; echo "Conectando VPN RO..."; for i in $(seq 1 20); do snxctl status 2>/dev/null | grep -qi connected && echo "✓ VPN RO conectada" && return 0; sleep 1; done; echo "✗ Timeout - verifique: snxctl status"; }
vpnpr() { snxctl disconnect 2>/dev/null; sleep 1; cp ~/.config/snx-rs/vpnpr.conf ~/.config/snx-rs/snx-rs.conf; snxctl connect; echo "Conectando VPN PR..."; for i in $(seq 1 20); do snxctl status 2>/dev/null | grep -qi connected && echo "✓ VPN PR conectada" && return 0; sleep 1; done; echo "✗ Timeout - verifique: snxctl status"; }
vpnam() { snxctl disconnect 2>/dev/null; sleep 1; cp ~/.config/snx-rs/vpnam.conf ~/.config/snx-rs/snx-rs.conf; snxctl connect; echo "Conectando VPN AM..."; for i in $(seq 1 20); do snxctl status 2>/dev/null | grep -qi connected && echo "✓ VPN AM conectada" && return 0; sleep 1; done; echo "✗ Timeout - verifique: snxctl status"; }
